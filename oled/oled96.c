...
static I2C_TypeDef* oledI2Cx;
static int oledI2cAddr;

static void oledWriteCommand(unsigned char);

void i2cWriteBytes(unsigned char *buf, int len)
{
	I2C_GenerateSTART(oledI2Cx, ENABLE);
	while (!I2C_CheckEvent(oledI2Cx, I2C_EVENT_MASTER_MODE_SELECT));

	I2C_Send7bitAddress(oledI2Cx, oledI2cAddr, I2C_Direction_Transmitter);
	while (!I2C_CheckEvent(oledI2Cx, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED));

//	I2C_SendData(I2Cx, iAddr);
//	while (!I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_BYTE_TRANSMITTED));

	for (int i = 0; i < len; i++) {
		I2C_SendData(oledI2Cx, buf[i]);
		while (!I2C_CheckEvent(oledI2Cx, I2C_EVENT_MASTER_BYTE_TRANSMITTED));
	}

	I2C_GenerateSTOP(oledI2Cx, ENABLE);
	while (!I2C_CheckEvent(oledI2Cx, I2C_EVENT_MASTER_BYTE_TRANSMITTED));
}
...
int oledInit(I2C_TypeDef* I2Cx, int iAddr, int iType, int bFlip, int bInvert)
{
...
//		write(file_i2c, oled64_initbuf, sizeof(oled64_initbuf));
		i2cWriteBytes(oled64_initbuf, sizeof(oled64_initbuf));
#if 0
		I2C_GenerateSTART(I2Cx, ENABLE);
		while (!I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_MODE_SELECT));
		I2C_Send7bitAddress(I2Cx, iAddr, I2C_Direction_Transmitter);
		while (!I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED));
		I2C_SendData(I2Cx, iAddr);
		while (!I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_BYTE_TRANSMITTED));
		for (int i = 0; i < sizeof(oled64_initbuf)) {
			I2C_SendData(I2Cx, oled64_initbuf);
			while (!I2C_CheckEvent(I2Cx, I2C_EVENT_MASTER_BYTE_TRANSMITTED));
		}
